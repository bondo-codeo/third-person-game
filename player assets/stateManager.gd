...
func idle():
	...
	elif Input.is_action_pressed("forward") and not Input.is_action_pressed("crouch"):
		changeState(groundStates.walking)
func walking():
	if Input.is_action_pressed("crouch"):
		changeState(groundStates.crouching)
	elif Input.is_action_pressed("run") and not Input.is_action_pressed("crouch"):
...
func running():
	if Input.is_action_pressed("crouch"):
		changeState(groundStates.crouching)
	elif Input.is_action_just_released("run") and state == groundStates.running:
		changeState(groundStates.walking)
